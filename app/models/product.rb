...
# Select pro.id, pro.code, pro.name, fam.name As family,
# if (pro.id in
# (Select Distinct prod.id
# From products prod
# Join location_items loi On loi.product_id = prod.id
# Join locations loc On loc.id = loi.location_id
# Where prod.type_id = 2 -- locação
# And (loc.ends_at Is Null Or loc.ends_at > Cast(Now() As Date)))
# ,"não","sim") As disponivel
# From products pro
# Left Join product_families fam On fam.id = pro.family_id;

  named_scope :grid, :select => "pro.id, pro.code, pro.name, fam.name As family, if (pro.id in (Select Distinct prod.id From products prod Join location_items loi On loi.product_id = prod.id Join locations loc On loc.id = loi.location_id Where prod.type_id = 2 And (loc.ends_at Is Null Or loc.ends_at > Cast(Now() As Date))) ,'não','sim') As disponivel",
