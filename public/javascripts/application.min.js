...
$(".buttonset").buttonset();$(".mask-phone").mask("(99) 9999-9999? r.9999",{placeholder:" "});$(".mask-cnpj").mask("99.999.999/9999-99",{placeholder:" "});$(".mask-code").mask("aaa.***.***.***",{placeholder:" "});$(".mask-cep").mask("99999-999",{placeholder:" "});$(".mask-cfop").mask("9999",{placeholder:" "});$(".mask-decimal").each(function(){var t=$(this);var d=t.attr("decimal");var conf={prefix:"",centsSeparator:",",thousandsSeparator:"."};if(d!==undefined){d=d.split(",");if(d.length==2){if(!isNaN(d[0]))conf.limit=
d[0]*1;if(!isNaN(d[1]))conf.centsLimit=d[1]*1}}t.priceFormat(conf)});$(".toggleable").each(function(){toggle=$(this);title=toggle.hide().prev();title.css("cursor","pointer");title.prepend('<span class="ui-icon ui-icon-triangle-1-e" style="display:inline-block"></span>');title.click(function(){t=$(this);t.next().slideToggle();ind=$("span",t);ind.toggleClass("ui-icon-triangle-1-e");ind.toggleClass("ui-icon-triangle-1-s")})});$(".datepicker").datepicker({"dateFormat":"yy-mm-dd"});$(".autocomplete").each(function(){var t=
$(this);var src=t.attr("source");var cbk=t.attr("callback");var pref=t.attr("pref");t.autocomplete({source:"/autocomplete/"+src,minLength:3,select:function(event,ui){$.each(ui.item,function(key,val){$("#"+pref+key).val(val)});eval(cbk)}})});message();make_buttons();limit_div_content()});$("body").ajaxError(function(jqXHR,textStatus,errorThrown){thisDialog=$("<div />",{"title":"Erro"}).appendTo(".ui-layout-center");thisDialog.append("<p>Ocorreu algo inesperado no sistema.</p>");thisDialog.append("<p>Informa\u00e7\u00f5es sobre este erro foram enviadas ao administrador do sistema.</p>");
